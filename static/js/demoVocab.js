...
var viewVocab = {
	...
	render: function() {
		this.imageWrapper.html('<img src="' + vocabController.getCurrentVocabImage() + '">');

		console.log(vocabData.list[vocabData.currentWordIndex].correct );
		// Hide Mic and show play button if player has already answered question
		if (vocabController.currentVocabCorrect()) {
			viewMic.hideMic();
			/*
			viewMic.hideMic();
			viewMic.revealPlayButton();
			*/
		} else {
			viewMic.revealMic();
			/*
			viewMic.hidePlayButton();
			viewMic.revealMic();
			*/
		}
	}
...
var viewMic = {
	...
	/* PERHAPS FOR FUTURE VERSION WHERE PLAYER CAN PLAY SOUNDS OF ALREADY ANSWERD QUESTIONS
	hideMic: function() {
		this.mic.children(".icon-mic").addClass('hidden');
	},
	revealMic: function() {
		this.mic.children(".icon-mic").removeClass('hidden');
	},
	revealPlayButton: function() {
		this.mic.children('.glyphicon-play').removeClass('hidden');
	},
	hidePlayButton: function() {
		this.mic.children('.glyphicon-play').addClass('hidden');
	},*/
	hideMic: function() {
