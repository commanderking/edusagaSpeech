...
export var SpeechChecker = {
	...
	checkAnswer: function(userAnswer, data, activeTaskIndex) {
		...
		possibleAnswers.forEach(function(possibleAnswerObject, i) {
			...
			var tempSoundID = possibleAnswerObject.soundID ? possibleAnswerObject.soundID:
				TaskController.getActiveTask(data,activeTaskIndex).soundID;
			...
			*/
			...
			if (exceptionFound === false) {
				...
				checkResult = possibleAnswerObject.answers[0].constructor === Array ? 
					that.advancedCheck(userAnswer, possibleAnswerObject) :
					that.typicalCheck(userAnswer, possibleAnswerObject)
